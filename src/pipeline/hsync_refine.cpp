#include "pipeline/hsync_refine.h"
#include <cmath>
#include <cstdio>
#include <algorithm>

// ============================================================================
// Helper functions (ported from cuVHS __device__ functions)
// ============================================================================

// Find falling-edge crossing: first sample where signal drops below target.
// Returns fractional sample position via linear interpolation, or -1.0 if not found.
static double find_falling_crossing(
    const double* sig,
    int start, int count,
    double target,
    int total_samples)
{
    if (start < 0 || start + count > total_samples || count < 2) return -1.0;

    for (int i = start + 1; i < start + count; i++) {
        double prev = sig[i - 1];
        double curr = sig[i];

        if (prev >= target && curr < target) {
            double a = prev - target;
            double b = curr - target;
            double frac = (a - b != 0.0) ? (a / (a - b)) : 0.0;
            return (double)(i - 1) + frac;
        }
    }
    return -1.0;
}

// Find rising-edge crossing: first sample where signal rises above target.
// Returns fractional sample position via linear interpolation, or -1.0 if not found.
// Matches Python calczc_do(..., edge=1): requires start sample to be below target.
static double find_rising_crossing(
    const double* sig,
    int start, int count,
    double target,
    int total_samples)
{
    if (start < 0 || start + count > total_samples || count < 2) return -1.0;

    // Python: if data[start] > target, return NONE (already past threshold)
    if (sig[start] > target) return -1.0;

    for (int i = start + 1; i < start + count; i++) {
        double prev = sig[i - 1];
        double curr = sig[i];

        if (prev < target && curr >= target) {
            double a = target - prev;
            double b = target - curr;
            double frac = (a + b != 0.0) ? (a / (a - b)) : 0.0;
            return (double)(i - 1) + frac;
        }
    }
    return -1.0;
}

// Compute median of a small window via partial selection sort.
// In practice these windows are 14-42 samples.
static double window_median(
    const double* sig,
    int start, int count,
    int total_samples)
{
    if (start < 0 || start + count > total_samples || count <= 0) return 0.0;

    double buf[64];
    int n = std::min(count, 64);
    for (int i = 0; i < n; i++) buf[i] = sig[start + i];

    // Partial sort to find median (only need to sort half)
    int mid = n / 2;
    for (int i = 0; i <= mid; i++) {
        for (int j = i + 1; j < n; j++) {
            if (buf[j] < buf[i]) {
                double tmp = buf[i];
                buf[i] = buf[j];
                buf[j] = tmp;
            }
        }
    }
    return buf[mid];
}

// ============================================================================
// Main refinement — process each line sequentially (was one GPU thread per line)
// ============================================================================

void hsync_refine(const double* demod05,
                  double* linelocs,
                  int total_demod_samples,
                  const VideoFormat& fmt)
{
    double one_usec_samples = 1.0e-6 * fmt.sample_rate;
    double sample_rate_mhz = fmt.sample_rate / 1.0e6;
    // [cuVHS calibration constant] right_edge_offset from Python vhs-decode
    double right_edge_offset = 2.25 * (sample_rate_mhz / 40.0);
    double pulse_threshold_hz = fmt.pulse_threshold_hz;
    double hsync_width_samples = fmt.hsync_width;
    int one_usec = (int)(one_usec_samples + 0.5);

    static int dbg_field = 0;
    for (int line = 0; line < fmt.lines_per_frame; line++) {
        if (line < fmt.active_line_start) continue;

        double original = linelocs[line];
        bool dbg = (dbg_field == 0 && line == 50);

        // -----------------------------------------------------------
        // Left-edge detection (falling edge = leading edge of hsync)
        // -----------------------------------------------------------
        int search_start = (int)(original - one_usec);
        int search_count = one_usec * 2;

        double zc = find_falling_crossing(demod05, search_start, search_count,
                                          pulse_threshold_hz, total_demod_samples);

        if (dbg) {
            fprintf(stderr, "  [HSYNC DBG] line 50: original=%.1f threshold=%.1f\n", original, pulse_threshold_hz);
            fprintf(stderr, "    search_start=%d count=%d total=%d\n", search_start, search_count, total_demod_samples);
            fprintf(stderr, "    demod05 around original: ");
            int oi = (int)original;
            for (int d = -3; d <= 3; d++)
                fprintf(stderr, "[%+d]=%.0f ", d, demod05[oi+d]);
            fprintf(stderr, "\n    zc=%.2f\n", zc);
        }

        double left_result = -1.0;

        if (zc >= 0.0) {
            int zc_int = (int)(zc + 0.5);

            // Sync level: 1.0 to 2.5 µs after crossing
            int sync_start = zc_int + (int)(1.0 * one_usec_samples);
            int sync_count = (int)(1.5 * one_usec_samples);
            double sync_level = window_median(demod05, sync_start, sync_count,
                                              total_demod_samples);

            // Porch level: 8 to 9 µs after crossing
            int porch_start = zc_int + (int)(8.0 * one_usec_samples);
            int porch_count = (int)(1.0 * one_usec_samples);
            double porch_level = window_median(demod05, porch_start, porch_count,
                                               total_demod_samples);

            if (dbg) {
                fprintf(stderr, "    sync_level=%.0f porch_level=%.0f porch>sync=%d\n",
                        sync_level, porch_level, porch_level > sync_level);
            }

            if (porch_level > sync_level) {
                double midpoint = (sync_level + porch_level) / 2.0;

                int refine_start = (int)(original - one_usec * 2);
                int refine_count = (int)(one_usec_samples * 4);

                double zc2 = find_falling_crossing(demod05, refine_start, refine_count,
                                                   midpoint, total_demod_samples);

                if (dbg) {
                    fprintf(stderr, "    midpoint=%.0f zc2=%.2f |zc2-zc|=%.2f limit=%.1f\n",
                            midpoint, zc2, zc2 >= 0 ? fabs(zc2 - zc) : -1.0, one_usec_samples * 0.5);
                }

                if (zc2 >= 0.0 && fabs(zc2 - zc) <= one_usec_samples * 0.5) {
                    left_result = zc2;
                }
            }
        }

        // -----------------------------------------------------------
        // Right-edge detection (rising edge = trailing edge of hsync)
        // Python: "less likely to be messed up by overshoot"
        // -----------------------------------------------------------
        double right_result = -1.0;

        int right_search_start = (int)(original + hsync_width_samples - one_usec_samples);
        int right_search_count = (int)(hsync_width_samples * 2);

        double right_cross = find_rising_crossing(demod05, right_search_start,
                                                  right_search_count,
                                                  pulse_threshold_hz,
                                                  total_demod_samples);

        if (right_cross >= 0.0) {
            // Derive estimated leading edge from right cross
            double zc_fr = right_cross - hsync_width_samples;
            int zc_fr_int = (int)(zc_fr + 0.5);

            // Sync level: 1.0 to 2.5 µs after derived leading edge
            int r_sync_start = zc_fr_int + (int)(1.0 * one_usec_samples);
            int r_sync_count = (int)(1.5 * one_usec_samples);
            double r_sync_level = window_median(demod05, r_sync_start, r_sync_count,
                                                total_demod_samples);

            // Porch level: hsync_width + 1µs to +2µs after derived leading edge
            int r_porch_start = zc_fr_int + (int)(hsync_width_samples + 1.0 * one_usec_samples);
            int r_porch_count = (int)(1.0 * one_usec_samples);
            double r_porch_level = window_median(demod05, r_porch_start, r_porch_count,
                                                 total_demod_samples);

            if (r_porch_level > r_sync_level) {
                double r_midpoint = (r_sync_level + r_porch_level) / 2.0;

                // Refine: find rising edge at midpoint threshold
                int r_refine_start = (int)(original + hsync_width_samples - one_usec_samples);
                int r_refine_count = (int)(hsync_width_samples * 2);

                double zc2_r = find_rising_crossing(demod05, r_refine_start,
                                                    r_refine_count, r_midpoint,
                                                    total_demod_samples);

                if (zc2_r >= 0.0 && fabs(zc2_r - right_cross) <= one_usec_samples * 0.5) {
                    // Derive lineloc from right edge
                    double candidate = right_cross - hsync_width_samples + right_edge_offset;

                    // Sanity: should be within ±2µs of left-edge result (or original)
                    double reference = (left_result >= 0.0) ? left_result : original;
                    if (fabs(candidate - reference) <= one_usec_samples * 2.0) {
                        right_result = candidate;
                    }
                }
            }
        }

        if (dbg) {
            fprintf(stderr, "    left_result=%.2f right_result=%.2f\n", left_result, right_result);
        }

        // -----------------------------------------------------------
        // Result selection: prefer right-edge (Python behavior)
        // -----------------------------------------------------------
        if (right_result >= 0.0) {
            linelocs[line] = right_result;
        } else if (left_result >= 0.0) {
            linelocs[line] = left_result;
        }
        // else: keep original
    }
    dbg_field++;
}
