...
// [NTSC-SPECIFIC] Field phase ID is an NTSC concept (1-4 cycle).
// PAL does not use field phase IDs — this call is a no-op for PAL.
...
bool TBCWriter::write_json() {
    ...
    // [NTSC-SPECIFIC] Active video region in output samples at 4×fsc.
    // These values will differ for PAL when that's implemented.
    ...
    for (int i = 0; i < field_count; i++) {
        ...
        // [NTSC-SPECIFIC] fieldPhaseID only written for NTSC (1-4 phase cycle).
